...
const Cart = () => {
  ...
  const [isValidating, setIsValidating] = useState(false);
  // NEW: Local state to track discount type ('percent' | 'fixed')
  // Ideally, this should also move to CartContext if you want it to persist across pages
  ...
  const handleApplyPromo = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/promocodes/validate/`, // Use env var
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        // Backend now returns: { code: "...", discount_value: 500, discount_type: "fixed", ... }
        setDiscount(Number(data.discount_value));
        setDiscountType(data.discount_type);

        if (data.discount_type === "fixed") {
          toast.success(
            `Promo code applied! You got ₹${data.discount_value} off.`
          );
        } else {
          toast.success(
            `Promo code applied! You got ${data.discount_value}% off.`
  ...
  // UPDATED: Calculate discount amount based on type
            ...
            <Card className="sticky top-24">
              <CardContent className="p-6">
                ...
                <div className="space-y-4 mb-6">
                  ...
                  {discount > 0 && (
                    <div className="flex justify-between text-primary animate-in fade-in slide-in-from-right-4">
                      {/* Dynamically show label based on type */}
                      <span>
                        Discount (
                        {discountType === "percent" ? `${discount}%` : "Flat"})
                      </span>
                      <span>
                        -₹
                        {discountAmount.toLocaleString(undefined, {
