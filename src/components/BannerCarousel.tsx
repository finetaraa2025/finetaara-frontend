    ...
    >
      {banners.map((banner, index) => (
        ...
        >
          ...
          {/* Text Content Layer - FIXED SPACING */}
          <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-16 pb-24 md:pb-28">
            {/* Mobile: Extra bottom padding for dots + button space */}
            <div className="max-w-3xl h-full flex flex-col justify-end pb-12 md:pb-0">
              <h2 className="text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-serif font-bold text-white mb-3 md:mb-4 animate-fade-in tracking-tight drop-shadow-lg leading-tight">
                {banner.title}
              </h2>
              <p className="text-lg sm:text-xl md:text-3xl text-gray-200 mb-6 md:mb-8 animate-fade-in font-light drop-shadow-md">
              ...
              <Link to="/collections">
                <Button
                  size="lg"
                  className="w-fit group shadow-lg hover:shadow-xl transition-all bg-primary hover:bg-primary/90 text-primary-foreground border-none mb-4 md:mb-0"
                >
                  <Sparkles className="mr-2 h-5 w-5 transition-transform group-hover:rotate-12" />
                  Explore Collections
                </Button>
      ...
      <Button
        ...
        size="icon"
        onClick={goToPrevious}
        className="hidden md:flex absolute left-2 md:left-4 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white backdrop-blur-md border border-white/20 transition-all hover:scale-110 z-20"
        aria-label="Previous banner"
      >
        <ChevronLeft className="h-5 w-5 md:h-6 md:w-6" />
      </Button>
      ...
      <Button
        ...
        size="icon"
        onClick={goToNext}
        className="hidden md:flex absolute right-2 md:right-4 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white backdrop-blur-md border border-white/20 transition-all hover:scale-110 z-20"
        aria-label="Next banner"
      >
        <ChevronRight className="h-5 w-5 md:h-6 md:w-6" />
      </Button>
      ...
      <div className="absolute bottom-6 md:bottom-8 left-1/2 -translate-x-1/2 z-30">
        <div className="flex gap-2 backdrop-blur-sm bg-black/30 px-4 py-2 rounded-full shadow-2xl border border-white/20">
          {banners.map((_, index) => (
            <button
              key={index}
              onClick={() => setCurrentIndex(index)}
              className={`h-3 w-3 md:h-2 md:w-2 rounded-full transition-all duration-300 hover:scale-110 ${
                index === currentIndex
